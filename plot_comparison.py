...
Objective: theoretical curves that MATCH the WNTR data, WITHOUT the cheating
of the original version.

Difference with the old version:
  - BEFORE: A relaxation ODE model (dP/dt) was invented, and the
    parameters were calibrated via regressions on the WNTR results, including
    an arbitrary P0 and empirical pressure/flow jumps.
  - HERE  : A purely PHYSICAL (algebraic) and STEADY-STATE model, typical
    of water networks (without unjustified differential equations):
        * P_static is dictated ONLY by the geometry (INP: Reservoir - Node).
        * R_eff (network resistance) is deduced from the nominal period: 
          P_static - P_w = R_eff * D^1.852.
        * The leak is modeled by the emitter law: Q_leak = K * sqrt(P).
        * Each leak event has its own physical characteristics 
          (breach size K_leak, and flow impact c_leak).
        * We solve P_m(t) = P_static - R_eff * (D(t) + Q_leak(t))^1.852 via
          scipy.optimize.fsolve (real implicit equation of hydraulics).
    We "invent nothing" (no arbitrary ODE, no non-physical parameters).
"""
import numpy as np
import pandas as pd
...
# 0. CONFIG
# ==============================================================
INP_PATH = 'city-pipelines/reseau/Scenario 3/Low_Demand.inp'
CSV_PATH = 'city-pipelines/reseau/Scenario 5/export_fuite_positive_3j.csv'
OBSERVED_PIPE = 'P76'
MCE_PER_BAR = 10.197
...
def prepare_network(wn):
    max_elev = max([n.elevation for _, n in wn.nodes() if hasattr(n, 'elevation')] + [0])
...
def load_wntr(csv_path):
    ...
    hours = (df['t'] / 3600.0 if df['t'].diff().max() > 2.0 else df['t'] / 20.0).to_numpy()
    ...
    demand = df['demande'].to_numpy() if 'demande' in df else np.full_like(hours, np.nan)
...
def calibrate(hours, P_w, Q_w, demand_w, leak, P_static):
    ...
    for start, end in leak_events:
        ...
        valid_P = P_f > 0
        K_l = float(np.median(Q_leak_est[valid_P] / np.sqrt(P_f[valid_P]))) if valid_P.any() else 0.0
        
        # Leak impact on Q76
        Q_nom_est = a * D_f + b
        dQ76 = Q_f - Q_nom_est
        valid_Qleak = Q_leak_est > 1e-6
        c_l = float(np.median(dQ76[valid_Qleak] / Q_leak_est[valid_Qleak])) if valid_Qleak.any() else 0.0
...
def main():
    ...
    for i, lk in enumerate(cal['leaks']):
        print(f"   Leak {i+1} (t={hours[lk['start']]:.1f}h to {hours[lk['end']]:.1f}h):")
    ...
    ax1.set_title(f"WNTR vs Physical Model (P_stat={cal['P_static']:.2f} bar, R_eff={cal['R_eff']:.2f})")
    ax1.legend()
