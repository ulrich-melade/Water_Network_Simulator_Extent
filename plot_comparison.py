...
# 0. CONFIG
# ==============================================================
INP_PATH = "city-pipelines/Scenarios/Scenario 3/Low_Demand.inp"
CSV_PATH = "city-pipelines/Network_Data/Scenario_b/Nominal_6d.csv"
OBSERVED_PIPE = "P76"
MCE_PER_BAR = 10.197
...
def load_wntr(csv_path):
    ...
    # Export layout is 4 columns per pipe: pressure, head loss, flow rate, friction.
    # Velocity is no longer exported; it is rebuilt from the flow rate in main().
    P = df["x_0"].to_numpy(dtype=float)
    HL = df["x_1"].to_numpy(dtype=float)
    ...
    return hours, P, HL, Q, demand, leak
...
def main():
    ...
    geo = load_geometry()
    hours, P_w, HL_w, Q_w, demand_w, leak = load_wntr(CSV_PATH)
    # Velocity is not exported any more: rebuild it from the flow rate (V = Q / A),
    # which is exactly how WNTR defines it. Q is exported in L/s.
    V_w = (Q_w / 1000.0) / geo["A"]
