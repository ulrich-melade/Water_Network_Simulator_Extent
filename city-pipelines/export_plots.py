...
SHOW_NAN_POINTS = True  # Set to True to display NaN points (missing data / broken sensors)
...
def plot_nan_indicators(ax, times, data):
    ...
    ax.plot(nan_times, [y_pos] * len(nan_times), 'rx', markersize=5, label='Broken sensor (NaN)', alpha=0.7)
...
for idx, pipe_id in enumerate(pipe_ids):
    ...
    if p_data_x:
        ax_p.plot(times, p_data_x, label=f'Real {pipe_id}', color='gray', linestyle='--', linewidth=1.5, alpha=0.8)

    if p_data_yc:
        ax_p.plot(times, p_data_yc, label=f'Measurement {pipe_id}', color=color_p, linewidth=2.5)
    ...
    if p_data_x:
        ...
        ax_p_zoom.plot(zoom_times, zoom_data_x, label=f'Real {pipe_id}', 
                       color='gray', linestyle='--', linewidth=1.5, alpha=0.8)
                       
    if p_data_yc:
        zoom_data_yc = p_data_yc[:len(zoom_times)]
        ax_p_zoom.plot(zoom_times, zoom_data_yc, label=f'Measurement {pipe_id}', 
    ...
    if idx == 0:
        ax_p_zoom.set_title("Pressure Zoom (0-24h)", fontsize=15, fontweight='bold', pad=15)
    ...
    if v_data_x:
        ...
        ax_v.plot(times, v_x_oriented, label=f'Real {pipe_id}', color='gray', linestyle='--', linewidth=1.5, alpha=0.8)

    if v_data_yc:
        v_yc_oriented = [v * sign_v if not np.isnan(v) else np.nan for v in v_data_yc]
        ax_v.plot(times, v_yc_oriented, label=f'Measurement {pipe_id}', color=color_v, linewidth=2.5)
    ...
    if q_data_x:
        ...
        ax_q.plot(times, q_x_ls, label=f'Real {pipe_id}', color='gray', linestyle='--', linewidth=1.5, alpha=0.8)

    if q_data_yc:
        q_yc_ls = [q * sign_q * 1000 if not np.isnan(q) else np.nan for q in q_data_yc]
        ax_q.plot(times, q_yc_ls, label=f'Measurement {pipe_id}', color=color_q, linewidth=2.5)
...
fig.suptitle("Water Network Simulation", fontsize=22, fontweight='bold', y=0.98, color='#2c3e50')
plt.tight_layout()
