...
CSV_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "Network", "Scenario", "3d_nominal.csv"
)
...
with open(CSV_PATH, "r") as f:
    ...
    # 3 columns per pipe (pressure, velocity, flow rate) — see simulator.append_to_csv
    x_cols = [h for h in header if h.startswith("x_")]
    num_pipes = len(x_cols) // 3
...
# 4 columns: Pressure, Pressure (Zoom), Velocity, Flowrate
...
for idx, pipe_id in enumerate(pipe_ids):
    ...
    # --- Velocity Subplot ---
    v_data_x = velocities.get(f"x_{base_idx + 1}", [])
    v_data_yc = velocities.get(f"yc_{base_idx + 1}", [])

    ax_v = axes[idx][2]
    # Nominal flow direction
    nom_v = [
        v
        for t, v in zip(times, v_data_x if v_data_x else v_data_yc)
        if t <= 24 and not np.isnan(v)
    ]
    ...
    if v_data_yc:
        ...
        if np.any(valid_v):
            ax_v.fill_between(
                np.array(times)[valid_v],
                v_data_clean[valid_v],
                color=color_v,
                alpha=0.15,
            )
        plot_nan_indicators(ax_v, times, v_yc_oriented)

    ax_v.set_ylabel("Velocity (m/s)", fontweight="bold", fontsize=11)
    ax_v.legend(loc="upper right", frameon=True, shadow=True, fancybox=True)

    if idx == 0:
        ax_v.set_title("Velocity Evolution", fontsize=15, fontweight="bold", pad=15)
