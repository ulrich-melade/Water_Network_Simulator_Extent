...
def pnoise2(x, y):
    # Simulate the behavior of pnoise2 (values between -1 and 1)
...
NODE_ZERO = "#6b7280"  # depression — gray
...
class App(ctk.CTk):
    ...
    def field(self, parent, row, col, label, placeholder):
        ...
        ctk.CTkLabel(
            f, text=label, font=ctk.CTkFont(size=11), text_color=TXT_SUB, anchor="w"
    ...
    def main(self):
        ...
        row_idx += 1
        self.optmenus["Head loss formula"] = self._optmenu(
            f3, 0, 0, "Head loss formula", ["Darcy-Weisbach", "Hazen-Williams"]
        )
        self.entries["Roughness"] = self.field(
            f3, 0, 1, "Roughness", "D-W: 0.02 | H-W: 130"
        )
        self.entries["Min diameter"] = self.field(
            f3, 1, 0, "Min diameter (mm)", "ex: 60"
        )
        self.entries["Max diameter"] = self.field(
            f3, 1, 1, "Max diameter (mm)", "ex: 300"
        )
        ...
        self.entries["Duration"] = self.field(f5, 0, 1, "Duration (h)", "ex: 12")
        ...
        row_idx += 1
        self.switches["Noise"] = self._switch(
            f6, 0, 0, "Enable Gaussian noise", command=self.toggle_noise
        )
        self.entries["Std dev"] = self.field(
            f6, 0, 1, "Noise standard deviation", "ex: 0.05"
        )
        self.entries["% broken"] = self.field(f6, 1, 0, "% broken sensors", "ex: 10")
        self.entries["Std dev"].configure(state="disabled")
    ...
    def canvas_panel(self):
        ...
        self._info_label = ctk.CTkLabel(
            bar,
            text="Generate a network then click on a node",
            font=ctk.CTkFont(size=11),
            text_color=TXT_MAIN,
        )
        ...
        ctk.CTkButton(
            ...
            border_color=BORDER,
            text_color=TXT_SUB,
            command=self.reset_faults,
        ).grid(row=0, column=3, padx=(0, 10))

        self.cv = tk.Canvas(
            right, bg=BG_DEEP, bd=0, highlightthickness=1, highlightbackground=BORDER
        )
        ...
        for col, txt in [
            ...
            tk.Canvas(f, width=10, height=10, bg=col, highlightthickness=0).pack(
                side="left", padx=(0, 3)
            )
            ctk.CTkLabel(
                f, text=txt, font=ctk.CTkFont(size=9), text_color=TXT_SUB
            ).pack(side="left")
    ...
    def _build_graph(self, params):
        ...
        if zone == "Urban":
            ...
            while len(edges_list) < n_houses:
                edges_list.extend(edges_list)
            ...
            junction_nodes = [
                n for n in G.nodes() if G.nodes[n].get("type") == "junction"
            ]
            if not junction_nodes:
                junction_nodes = list(G.nodes())
            ...
            for i in range(min(n_branches, len(branch_nodes))):
                ...
                for step in range(b_len):
                    ...
                    offset = np.array(
                        [random.uniform(-0.1, 0.1), side * random.uniform(0.2, 0.4)]
                    )
        ...
        if pct_broken > 0:
            nodes_to_break = random.sample(
                list(G.nodes()), int(len(G.nodes()) * (pct_broken / 100.0))
            )
    ...
    def _draw_node(self, node):
        ...
        if state != "normal":
            ...
            color = {"broken": "red", "surge": "orange", "zero": "black"}.get(
                state, "red"
            )
        else:
            # Our "design" colors for normal operation
            color = {"tower": "#0B97F4", "connection": "gray", "house": "#CD5C5C"}.get(
                ntype, "#11D454"
            )

        # --- DRAWING ACCORDING TO TYPE ---
        ...
        if ntype == "tower":
            ...
            # Houses (M1, M2...): Large red circle, WHITE text in the center
            r = 12
            self.cv.create_oval(
                x - r,
                y - r,
    ...
    def _draw_empty(self):
        ...
        self.cv.create_text(
            w // 2,
            h // 2,
            text=" Network not generated\n\nConfigure the parameters on the left\nthen click 'Generate network'",
            fill=TXT_MAIN,
    ...
    def _show_fault_popup(self, node):
        ...
        popup.title(f"{'House' if ntype == 'house' else 'Node'}  {node}")
        ...
        if ntype == "house":
            ...
            ctk.CTkLabel(
                popup,
                text=f"Attached to junction {parent}",
        ...
        ctk.CTkLabel(
            popup,
            text=f"Current state: {self._node_state.get(node, 'normal')}",
            font=ctk.CTkFont(size=10),
            text_color=TXT_SUB,
        ).pack(pady=(0, 10))
        ...
        for label, state_key, border in [
            ...
            ctk.CTkButton(
                ...
                border_width=1,
                border_color=border,
                command=lambda s=state_key: apply(s),
    ...
    def toggle_noise(self):
        self.entries["Std dev"].configure(
            state="normal" if self.switches["Noise"].get() else "disabled"
        )
    ...
    def _collect(self):
        return {
            ...
            "formula": self.optmenus["Head loss formula"].get(),
            "roughness": self.entries["Roughness"].get(),
            "diam_min": self.entries["Min diameter"].get(),
            "diam_max": self.entries["Max diameter"].get(),
            "sim_mode": self.optmenus["Simulation mode"].get(),
            "duration": self.entries["Duration"].get(),
            "noise": self.switches["Noise"].get(),
            "std_dev": self.entries["Std dev"].get(),
            "pct_broken": self.entries["% broken"].get(),
    ...
    def on_ok(self):
        ...
        try:
            ...
            self.feedback.configure(
                text="Invalid number of water towers", text_color=NODE_FAULT
            )
            return
        ...
        n_houses = sum(
            1 for _, d in self._G.nodes(data=True) if d.get("type") == "house"
        )
        self.feedback.configure(
        ...
        self._info_label.configure(
            text="Click on a house or junction to inject a fault", text_color=TXT_MAIN
        )
    ...
    def on_random(self):
        ...
        rand_params = {
            ...
            "Min diameter": str(rng.choice([50, 80, 100])),
            "Max diameter": str(rng.choice([200, 250, 300])),
            "Duration": str(rng.choice([0, 6, 12, 24])),
            "% broken": str(rng.randint(0, 20)),
        ...
        self.optmenus["Head loss formula"].set("Darcy-Weisbach")
        self.optmenus["Simulation mode"].set(
            rng.choice(["Static (0h)", "Dynamic (24h)"])
        )
        self.feedback.configure(
            text="🎲  Random values generated", text_color="#3dba6f"
        )
        self.on_ok()
    ...
    def export_inp(self):
        if not self._G:
            self.feedback.configure(
                text="⚠  Generate a network first", text_color=NODE_FAULT
            )
            return
        ...
        try:
            ...
            from optimize_network import (
                optimize_network,
                modify_global_demand,
                restore_original_demands,
            )

            self.feedback.configure(
                text=f"⚙  Generating scenarios and Optimizing...", text_color="#f39c12"
            )
            self.update()
            ...
            self.feedback.configure(
                text=f"✓  Exported: {basename} + Low/High Demand", text_color="#3dba6f"
            )
        except Exception as e:
            self.feedback.configure(
                text=f"⚠  Error during optimization : {e}", text_color="#e74c3c"
            )
    ...
    def export_image(self):
        if not self._G:
            self.feedback.configure(
                text="⚠  Generate a network first", text_color="#e74c3c"
            )
            return
        path = filedialog.asksaveasfilename(
        ...
        self.feedback.configure(
            text="⚙  Generating high resolution image...", text_color="#f39c12"
        )
        self.update()
        ...
        try:
            ...
            nx.draw_networkx_edge_labels(
                self._G, pos_plt, edge_labels=edge_labels, font_size=6, font_color="red"
            )
            ...
            self.feedback.configure(
                text=f"✓  Image saved : {path.split('/')[-1]}", text_color="#3dba6f"
            )
        except Exception as e:
            self.feedback.configure(
                text=f"⚠  Error during export : {e}", text_color="#e74c3c"
            )
    ...
    def _build_inp(self):
        ...
        w("[TITLE]")
        w(
            f"; Zone={p.get('zone')} | Houses={p.get('n_houses')} | Water towers={p.get('n_towers')}"
        )
        w(f"; Active faults : {faults if faults else 'none'}")
        w("")
        ...
        all_juncs = [
            (n, d) for n, d in self._G.nodes(data=True) if d.get("type") == "junction"
        ]
        all_houses = [
            (n, d) for n, d in self._G.nodes(data=True) if d.get("type") == "house"
        ]
        ...
        for i, (node, _) in enumerate(all_juncs):
            ...
            if pos_xy is not None:
                x, y = pos_xy
                elev = round(
                    get_elevation(x / 800.0, y / 800.0, min_height, max_height), 2
                )
            else:
        ...
        for i, (node, _) in enumerate(all_houses):
            ...
            demand = (
                round(house_demand + 5.0, 5)
                if state == "surge"
                else round(house_demand, 5)
            )
            w(f" {node:<16}\t{elev:<12}\t{demand:<12}\t                \t;  [{state}]")
        w("")
        ...
        w(";ID              \tHead        \tPattern")
        towers = [
            (n, d) for n, d in self._G.nodes(data=True) if d.get("type") == "tower"
        ]
        for node, _ in towers:
            ...
            if pos_xy is not None:
                ...
                head = round((min_height + max_height) / 2, 2)
            w(f" {node:<16}\t{head:<12}\t \t;")
        w("")

        w("[TANKS]")
        w(
            ";ID  \tElev  \tInitLevel  \tMinLevel  \tMaxLevel  \tDiameter  \tMinVol  \tVolCurve"
        )
        w("")
        ...
        for k, (u, v, ed) in enumerate(self._G.edges(data=True)):
            ...
            if type_u == "tower" or type_v == "tower":
                ...
                pipes_lines.append(
                    f" P{k + 1:<15}\t{n1:<16}\t{n2:<16}\t{len_main:<12}\t{diam:<12}\t{rug:<12}\t0 \tOpen \t;"
                )
                continue
            ...
            # If house -> min_diam
                ...
                length = len_sec
                pipes_lines.append(
                    f" P{k + 1:<15}\t{u:<16}\t{v:<16}\t{length:<12}\t{diam:<12}\t{rug:<12}\t0 \tOpen \t;"
                )
                continue
            ...
            # If other junctions -> intermediate diam
                ...
                if abs(elev_u - elev_v) > 15.0 and random.random() < 0.15:
                    ...
                    valves_lines.append(
                        f" V{k + 1:<15}\t{n1:<16}\t{n2:<16}\t{diam:<10}\tPRV \t30 \t0"
                    )
                else:
                    pipes_lines.append(
                        f" P{k + 1:<15}\t{u:<16}\t{v:<16}\t{length:<12}\t{diam:<12}\t{rug:<12}\t0 \tOpen \t;"
